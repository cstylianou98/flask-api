...
from application.models import FriendCharacter
...
def create_character():
    data = request.json
    character = FriendCharacter(data['name'], data['age'], data['catch_phrase'])
...
def get_characters():
    characters= FriendCharacter.query.all()
    character_list = []
...
def get_character(id):
    character = FriendCharacter.query.filter_by(id=id).first()
    return jsonify(id=character.id, name=character.name, age=character.age, catch_phrase = character.catch_phrase)
...
def update_character(id):
    character= FriendCharacter.query.filter_by(id=id)
    data = request.json
    character.update(dict(name=data['name'], age=data['age'], catch_phrase = data['catch_phrase']))
    db.session.commit()
    updatedCharacter = character.first() #retrieving character
